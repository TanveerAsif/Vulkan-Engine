...
GraphicsPipeline::GraphicsPipeline(VkDevice device, GLFWwindow* window, VkRenderPass renderPass, VkShaderModule vs,
                                   VkShaderModule fs, const SimpleMesh* pMesh, int32_t numSwapchainImages,
                                   const std::vector<BufferAndMemory>& uniformBuffers, size_t uniformBufferSize,
                                   bool depthEnabled)
    : mDevice(device), mPipelineLayout(VK_NULL_HANDLE), mGraphicsPipeline(VK_NULL_HANDLE)
{

    if (pMesh)
    {
        createDescriptorSets(pMesh, numSwapchainImages, uniformBuffers, uniformBufferSize);
    }
    ...
    VkGraphicsPipelineCreateInfo pipelineInfo = {.sType = VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO,
                                                 ...
                                                 .pDepthStencilState =
                                                     depthEnabled ? &depthStencilState : VK_NULL_HANDLE,
...
void GraphicsPipeline::createDescriptorSets(const SimpleMesh* mesh, int32_t numSwapchainImages,
                                            ...
                                            size_t uniformBufferSize)
{
    ...
    updateDescriptorSets(mesh, numSwapchainImages, uniformBuffers, uniformBufferSize);
}
...
{
    std::vector<VkDescriptorSetLayout> layouts(numSwapchainImages, mDescriptorSetLayout);
...
void GraphicsPipeline::updateDescriptorSets(const SimpleMesh* mesh, int32_t numSwapchainImages,
                                            const std::vector<BufferAndMemory>& uniformBuffers,
                                            size_t uniformBufferSize)
{
