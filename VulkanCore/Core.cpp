...
VulkanCore::~VulkanCore()
{
    ...
    for (int32_t i = 0; i < static_cast<int32_t>(mSwapchainImageViews.size()); ++i)
    {
    ...
    {
        auto func = (PFN_vkDestroyDebugUtilsMessengerEXT)vkGetInstanceProcAddr(mVulkanInstance,
...
void VulkanCore::initialize(std::string appName, GLFWwindow* window, bool depthEnabled)
{
...
void VulkanCore::createDebugCallback()
{
    ...
    VkDebugUtilsMessengerCreateInfoEXT messangerCreateInfo = {
        ...
        .pfnUserCallback = [](VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
            ...
            std::cout << "Message Severity: " << messageSeverity << ", MessageType: " << messageType << std::endl;
            std::cout << "objectType: ";
    ...
    if (func != nullptr)
    {
        if (func(mVulkanInstance, &messangerCreateInfo, nullptr, &mDebugMessenger) != VK_SUCCESS)
        {
...
{
    if (glfwCreateWindowSurface(mVulkanInstance, window, nullptr, &mSurface) != VK_SUCCESS)
    {
...
{
    const auto& physicalDeviceProps = mPhysicalDevice.getSelectedPhysicalDeviceProperties();
    ...
    VkPhysicalDeviceFeatures deviceFeatures = {
        .geometryShader = VK_TRUE, .tessellationShader = VK_TRUE, .samplerAnisotropy = VK_TRUE};
    ...
    {
        throw std::runtime_error("Selected physical device does not support geometry shaders.");
    }
    if (physicalDeviceProps.mFeatures.tessellationShader == VK_FALSE)
    {
        throw std::runtime_error("Selected physical device does not support tessellation shaders.");
    }
    if (physicalDeviceProps.mFeatures.samplerAnisotropy == VK_FALSE)
    {
        throw std::runtime_error("Selected physical device does not support sampler anisotropy.");
    }

    std::vector<const char*> deviceExtensions = {// swapchain extension is required for any windowed application
    ...
    if (vkCreateDevice(physicalDeviceProps.mPhysicalDevice, &deviceCreateInfo, nullptr, &mLogicalDevice) != VK_SUCCESS)
    {
...
void VulkanCore::createSwapChain()
{
    ...
    const uint32_t numImages = [&surfaceCaps]() -> uint32_t {
        ...
        if (surfaceCaps.maxImageCount > 0 && requiredImageCount > surfaceCaps.maxImageCount)
    ...
    for (uint32_t i{0}; i < actualImageCount; ++i)
    {
        ...
        VkResult result = vkCreateImageView(mLogicalDevice, &viewInfo, nullptr, &mSwapchainImageViews[i]);
        if (result != VK_SUCCESS)
        {
...
void VulkanCore::createCommandBuffers(VkCommandBuffer* commandBuffers, int32_t count)
{
    VkCommandBufferAllocateInfo cmdBufAllocInfo = {
        .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,
        .pNext = nullptr,
        .commandPool = mCommandPool,
        .level = VK_COMMAND_BUFFER_LEVEL_PRIMARY, // primary command buffer can be
    ...
    if (vkAllocateCommandBuffers(mLogicalDevice, &cmdBufAllocInfo, commandBuffers) != VK_SUCCESS)
    {
        throw std::runtime_error("Failed to allocate command buffers!");
    }

    std::cout << "Allocated " << count << " command buffers successfully." << std::endl;
}
...
void VulkanCore::freeCommandBuffers(VkCommandBuffer* commandBuffers, int32_t count)
{
    ...
    vkFreeCommandBuffers(mLogicalDevice, mCommandPool, static_cast<uint32_t>(count), commandBuffers);
}
...
VkRenderPass VulkanCore::createSimpleRenderPass()
{
    ...
    VkAttachmentDescription colorAttachDesc = {
        ...
        .finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR // swapchain image layout to
                                                       // transition to after the render pass
    };

    VkAttachmentReference colorAttachRef = {.attachment = 0, .layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL};

    VkFormat depthFormat = mPhysicalDevice.getSelectedPhysicalDeviceProperties().mDepthFormat;

    VkAttachmentDescription depthAttachmentDesc = {.flags = 0,
                                                   .format = depthFormat,
    ...
    VkAttachmentReference depthAttachRef = {.attachment = 1,
...
BufferAndMemory VulkanCore::createVertexBuffer(const void* pVertices, size_t size)
{
    ...
    if (vkMapMemory(mLogicalDevice, stagingVB.mMemory, offset, stagingVB.mAllocationSize, flags, &pData) != VK_SUCCESS)
    {
                                         ...
                                         VkMemoryPropertyFlags reqMemPropFlags)
{
    ...
    // Create buffer
    VkBufferCreateInfo vbCreateInfo = {.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,
                                       .pNext = nullptr,
                                       .flags = 0,
                                       .size = size,
                                       .usage = usage,
    ...
    if (vkCreateBuffer(mLogicalDevice, &vbCreateInfo, nullptr, &bufferAndMemory.mBuffer) != VK_SUCCESS)
    {
    ...
    if (vkAllocateMemory(mLogicalDevice, &allocInfo, nullptr, &bufferAndMemory.mMemory) != VK_SUCCESS)
    {
        throw std::runtime_error("Failed to allocate buffer memory!");
    }

    // Step 5. Bind buffer and memory
    vkBindBufferMemory(mLogicalDevice, bufferAndMemory.mBuffer, bufferAndMemory.mMemory, 0);
...
uint32_t VulkanCore::getMemoryTypeIndex(uint32_t typeFilter, VkMemoryPropertyFlags reqMemPropFlags)
{
    ...
    for (uint32_t i = 0; i < memProperties.memoryTypeCount; i++)
    {
        ...
        bool hasRequiredProperties = ((memType.propertyFlags & reqMemPropFlags) == reqMemPropFlags);
    ...
    std::cout << "Can't find suitable memory type! typeFilter: " << typeFilter << ", properties: " << reqMemPropFlags
              << std::endl;
    return -1;
}
...
void BufferAndMemory::update(VkDevice device, const void* pData, VkDeviceSize size)
{
...
{
    const int32_t numSwapChainImages = static_cast<int32_t>(mSwapchainImages.size());
