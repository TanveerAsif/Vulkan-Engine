                        ...
                        ImS64 size_contents_v, ImDrawFlags draw_rounding_flags)
{
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        const int held_dir =
            (clicked_v_norm < grab_v_norm) ? -1 : (clicked_v_norm > grab_v_norm + grab_h_norm) ? +1 : 0;
        if (g.ActiveIdIsJustActivated)
        {
                          ...
                          const char* format, ImGuiSliderFlags flags)
{
    ...
    else if (g.ActiveIdSource == ImGuiInputSource_Keyboard || g.ActiveIdSource == ImGuiInputSource_Gamepad)
    {
        ...
        const float tweak_factor =
            (flags & ImGuiSliderFlags_NoSpeedTweaks) ? 1.0f : tweak_slow ? 1.0f / 10.0f : tweak_fast ? 10.0f : 1.0f;
    ...
    if (is_visible)
    {
        ...
        if (highlighted || selected)
        {
            ...
            ImU32 col = GetColorU32((held && highlighted) ? ImGuiCol_HeaderActive
                                                          : highlighted ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
...
static void BoxSelectScrollWithMouseDrag(ImGuiBoxSelectState* bs, ImGuiWindow* window, const ImRect& inner_r)
{
    ...
    for (int n = 0; n < 2; n++) // each axis
    {
        ...
        const float dist = (mouse_pos > inner_r.Max[n])
                               ...
                               : (mouse_pos < inner_r.Min[n]) ? mouse_pos - inner_r.Min[n] : 0.0f;
