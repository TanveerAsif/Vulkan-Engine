...
typedef int ImGuiWindowBgClickFlags; // -> enum ImGuiWindowBgClickFlags_ // Flags: for overriding behavior of clicking
                                     // on window background/void.
...
ImTextCalcWordWrapNextLineStart(const char* text, const char* text_end,
...
enum ImGuiWindowBgClickFlags_
{
    ...
    ImGuiWindowBgClickFlags_Move = 1 << 0, // Click on bg/void + drag to move window. Cleared by default when using
...
struct ImGuiNextItemData
{
    ...
    ImU32 ColorMarker; // Set by SetNextItemColorMarker(). Not exposed yet, supported by DragScalar,SliderScalar and for
...
//inconsistent with our pattern that _None values == 0 and quite dangerous. Also using _NoOwner makes the IsKeyPressed()
//calls more explicit.
...
struct ImGuiContext
{
    ...
    // (2024/10/18). OPPOSITE VALUE (g.NavDisableHighlight == !g.NavCursorVisible) bool NavDisableMouseHover; // Old name
    // for g.NavHighlightItemUnderNav before 1.91.1 (2024/10/18) this was called When user starts using keyboard/gamepad,
    // we hide mouse hovering highlight until mouse is touched again.
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImGuiWindowBgClickFlags
        BgClickFlags : 8; // Configure behavior of click+dragging on window bg/void or over items. Default sets by
    ...
    ImGuiWindow* ParentWindowForFocusRoute; // Set to manual link a window to its logical parent so that Shortcut()
                                            // chain are honored (e.g. Tool linked to Document)
