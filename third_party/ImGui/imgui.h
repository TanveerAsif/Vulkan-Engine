...
TableNextRow(ImGuiTableRowFlags row_flags = 0,
             float min_row_height = 0.0f); // append into the first cell of a new row. 'min_row_height' include the
...
// ImGuiChildFlags_NavFlattened, 0) ImGuiWindowFlags_AlwaysUseWindowPadding = 1 << 30,  // Obsoleted in 1.90.0: moved to
// ImGuiChildFlags. BeginChild(name, size, 0, ImGuiWindowFlags_AlwaysUseWindowPadding) --> BeginChild(name, size,
...
enum ImGuiSliderFlags_
{
    ...
    ImGuiSliderFlags_InvalidMask_ =
        0x7000000F, // [Internal] We treat using those bits as being potentially a 'float power' argument from legacy
                    // API (obsoleted 2020-08) that has got miscast to this enum, and will trigger an assert if needed.
};
...
struct ImGuiListClipper
{
    ...
    // items_height = -1.0f) { memset(this, 0, sizeof(*this)); ItemsCount = -1; Begin(items_count, items_height); } //
    // [removed in 1.79]
#endif
};
...
struct ImDrawList
{
    ...
    // num_segments); } // OBSOLETED in 1.90.5 (Mar 2024) inline  void  AddBezierCurve(const ImVec2& p1, const ImVec2&
    // p2, const ImVec2& p3, const ImVec2& p4, ImU32 col, float thickness, int num_segments = 0) { AddBezierCubic(p1, p2,
    // p3, p4, col, thickness, num_segments); }                         // OBSOLETED in 1.80 (Jan 2021) inline  void
...
IMGUI_API void
SetWindowFontScale(float scale); // Set font scale factor for current window. Prefer using PushFont(NULL,
...
// ImGuiChildFlags_Borders : ImGuiChildFlags_None, window_flags);     } // Unnecessary as true == ImGuiChildFlags_Borders
...
// tint_col = ImVec4(1, 1, 1, 1)); // --> Use new ImageButton() signature (explicit item id, regular FramePadding). Refer
...
// (ImGuiKey)(key_data - g.IO.KeysData); } // Map ImGuiKey_* values into legacy native key index. == io.KeyMap[key]. When
// using a 1.87+ backend using io.AddKeyEvent(), calling GetKeyIndex() with ANY ImGuiKey_XXXX values will return the same
...
// BeginListBox(label, size); } static inline bool  ListBoxHeader(const char* label, int items_count, int height_in_items
// = -1) { float height = GetTextLineHeightWithSpacing() * ((height_in_items < 0 ? ImMin(items_count, 7) :
...
// format, float power = 1.0f)                                                            // OBSOLETED in 1.78 (from June
// 2020) IMGUI_API bool      DragScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, float
// v_speed, const void* p_min, const void* p_max, const char* format, float power = 1.0f); // OBSOLETED in 1.78 (from
// June 2020) IMGUI_API bool      SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void*
// p_min, const void* p_max, const char* format, float power = 1.0f); // OBSOLETED in 1.78 (from June 2020) IMGUI_API
// bool      SliderScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, const void* p_min,
// const void* p_max, const char* format, float power = 1.0f);                                                       //
...
// = 1.0f) { return DragScalarN(label, ImGuiDataType_Float, v, 2, v_speed, &v_min, &v_max, format, power); } // OBSOLETED
// in 1.78 (from June 2020) static inline bool  DragFloat3(const char* label, float v[3], float v_speed, float v_min,
// float v_max, const char* format, float power = 1.0f) { return DragScalarN(label, ImGuiDataType_Float, v, 3, v_speed,
// &v_min, &v_max, format, power); } // OBSOLETED in 1.78 (from June 2020) static inline bool  DragFloat4(const char*
// label, float v[4], float v_speed, float v_min, float v_max, const char* format, float power = 1.0f) { return
// DragScalarN(label, ImGuiDataType_Float, v, 4, v_speed, &v_min, &v_max, format, power); } // OBSOLETED in 1.78 (from
// June 2020) static inline bool  SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format,
// float power = 1.0f)                 { return SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format,
// power); }            // OBSOLETED in 1.78 (from June 2020) static inline bool  SliderFloat2(const char* label, float
// v[2], float v_min, float v_max, const char* format, float power = 1.0f)              { return SliderScalarN(label,
// ImGuiDataType_Float, v, 2, &v_min, &v_max, format, power); }        // OBSOLETED in 1.78 (from June 2020) static
// inline bool  SliderFloat3(const char* label, float v[3], float v_min, float v_max, const char* format, float power
// = 1.0f)              { return SliderScalarN(label, ImGuiDataType_Float, v, 3, &v_min, &v_max, format, power); } //
// OBSOLETED in 1.78 (from June 2020) static inline bool  SliderFloat4(const char* label, float v[4], float v_min, float
// v_max, const char* format, float power = 1.0f)              { return SliderScalarN(label, ImGuiDataType_Float, v, 4,
...
// BeginPopupContextWindow(str_id, mb | (over_items ? 0 : ImGuiPopupFlags_NoOpenOverItems)); } // OBSOLETED in 1.77 (from
// June 2020) static inline void  TreeAdvanceToLabelPos()               { SetCursorPosX(GetCursorPosX() +
...
// static inline float GetItemsLineHeightWithSpacing()       { return GetFrameHeightWithSpacing(); } // OBSOLETED
// in 1.53 (between Oct 2017 and Dec 2017) IMGUI_API bool      Begin(char* name, bool* p_open, ImVec2 size_first_use,
// float bg_alpha = -1.0f, ImGuiWindowFlags flags=0); // OBSOLETED in 1.52 (between Aug 2017 and Oct 2017): Equivalent of
// using SetNextWindowSize(size, ImGuiCond_FirstUseEver) and SetNextWindowBgAlpha(). static inline bool
...
// AlignTextToFramePadding(); }                                      // OBSOLETED in 1.52 (between Aug 2017 and Oct 2017)
// static inline void  SetNextWindowPosCenter(ImGuiCond c=0) { SetNextWindowPos(GetMainViewport()->GetCenter(), c,
// ImVec2(0.5f,0.5f)); } // OBSOLETED in 1.52 (between Aug 2017 and Oct 2017) static inline bool  IsItemHoveredRect() {
// return IsItemHovered(ImGuiHoveredFlags_RectOnly); }               // OBSOLETED in 1.51 (between Jun 2017 and Aug 2017)
...
// IsWindowHovered(ImGuiHoveredFlags_AnyWindow); }            // OBSOLETED in 1.51 (between Jun 2017 and Aug 2017) static
// inline bool  IsMouseHoveringWindow()               { return IsWindowHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup
